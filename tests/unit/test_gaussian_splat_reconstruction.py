...
from fvdb.utils.metrics import ssim
...
class TestMaskRasterizationConfig(unittest.TestCase):
    ...
    def test_rejects_unsupported_combinations(self):
        ...
        for overrides, message in cases:
            with self.subTest(overrides=overrides):
                values = {"mask_rasterization_mode": "bbox", **overrides}
    ...
    def test_rejects_non_rgb_model_and_unknown_mode(self):
        config = GaussianSplatReconstructionConfig(mask_rasterization_mode="tiles")
...
class TestMaskRasterizationTrainingLoopWiring(unittest.TestCase):
    def test_bbox_uses_full_sensor_crop_and_area_scaled_image_objective(self):
        ...
        logged_metrics = {
            ...
            for call in runner._writer.log_metric.call_args_list
            if len(call.args) == 3
        }
...
class TestMaskRasterizationLossParity(unittest.TestCase):
    ...
    def _image_objective(
        ...
        masked_target = _masked_ground_truth(target, prediction, valid_mask)
        l1loss = nnf.l1_loss(prediction, masked_target)
        ssimloss = 1.0 - ssim(
            prediction.permute(0, 3, 1, 2).contiguous(),
            masked_target.permute(0, 3, 1, 2).contiguous(),
        )
